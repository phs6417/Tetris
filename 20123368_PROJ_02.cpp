...
mvwhline(win_, 21, 0, ACS_LLCORNER, 1);
mvwhline(win_, 21,21, ACS_LRCORNER, 1);
for (int i=0; i<height_-6;i++)
mvwhline(win_, i+1, 1, ACS_DIAMOND, width_-2);
...
for(int i=xloc;i<xloc+2;i++){
	for(int j=yloc;j<yloc+4;j++){
		//mvwhline(win_, i+1, j-39, ACS_CKBOARD, width_-2);
		mvaddstr(i,j,"1");
...
/*
class Block : public Pane {
public: 
Block(int x, int y, int w, int h) : Pane(x,y,w,h){}
...
boardPane_ = new BoardPane(30,0,22,26);
